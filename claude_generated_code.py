...
Available tools: """ + str(available_tools or [])
    ...
    def plan_execution(self, analysis: Dict[str, Any], user_request: str, 
        ...
        system_prompt = f"""You are an advanced execution planner for AI agents. Create detailed execution plans with:

1. Step-by-step breakdown
2. Decision points and alternatives
3. Error handling and fallbacks
4. Success criteria for each step
5. Dependencies and prerequisites
6. Quality checkpoints

Consider the task complexity ({complexity}) and type ({primary_task}).
...
class EnhancedGeneralGroqAgent:
        ...
        for step, result in zip(plan, results):
            if result.success:
                if step.action == "filesystem":
                    if step.operation == "create_file" and result.data:
                        file_path = result.data.get("path", "file")
                        size = result.data.get("size", 0)
                        accomplishments.append(f"Created {file_path} ({size} bytes)")
                    elif step.operation == "read_file" and result.data:
                        file_path = result.data.get("path", "file")
                        lines = result.data.get("lines", 0)
                        accomplishments.append(f"Read {file_path} ({lines} lines)")
                    elif step.operation == "list_directory" and result.data:
                        total_items = result.data.get("total_items", 0)
                        files = result.data.get("files", 0)
                        dirs = result.data.get("directories", 0)
                        accomplishments.append(f"Listed directory: {files} files, {dirs} directories")
                
                elif step.action == "command":
                    if result.data:
                        cmd = result.data.get("command", "command")[:50]
                        accomplishments.append(f"Executed: {cmd}")
                
                elif step.action == "web" and result.data:
                    content_length = result.data.get("content_length", 0)
                    accomplishments.append(f"Web search completed ({content_length} chars retrieved)")
                
                elif step.action == "data_processing" and result.data:
                    if "parsed" in result.data:
                        accomplishments.append("Data successfully parsed and analyzed")
                    elif "statistics" in str(result.data):
                        accomplishments.append("Statistical analysis completed")
                
                elif step.action == "llm_generation" and result.data:
    ...
    def _learn_from_execution(self, user_input: str, state: Dict[str, Any]):
        ...
        if state.get("goal_met"):
            task_type = None
            if state["history"]:
                task_type = state["history"][0].get("analysis", {}).get("primary_task_type")
            
            if task_type:
                # Update successful patterns
                if task_type not in self.common_patterns:
                    self.common_patterns[task_type] = []
                
                pattern = {
                    "input_keywords": user_input.lower().split()[:5],
                    "successful_tools": list(set([
                        step.action for history in state["history"] 
                        for step in history.get("plan", [])
                    ])),
                    "iterations_needed": len(state["history"]),
                    "quality_score": state.get("quality_assessment", {}).get("overall_quality", 0.5)
                }
                
                self.common_patterns[task_type].append(pattern)
                
                # Keep only recent patterns (last 10)
                if len(self.common_patterns[task_type]) > 10:
                    self.common_patterns[task_type] = self.common_patterns[task_type][-10:]
        ...
        if failed_steps:
            error_signature = str(hash(str(failed_steps)))
    ...
    def get_performance_insights(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "learned_patterns": {k: len(v) for k, v in self.common_patterns.items()},
            "error_patterns": {k: v["frequency"] for k, v in self.error_patterns.items()},
            "decision_history_count": len(self.decision_history),
            "avg_quality_score": (
                sum(self.performance_metrics["quality_scores"]) / 
                max(len(self.performance_metrics["quality_scores"]), 1)
            ) if self.performance_metrics["quality_scores"] else 0.0
        }
    
    def get_available_tools(self) -> Dict[str, str]:
        """Get available tools and their descriptions with capabilities"""
        tool_info = {}
    ...
    def get_execution_history(self, limit: int = 5) -> List[Dict[str, Any]]:
        """Get recent execution history with enhanced details"""
        recent_history = self.execution_history[-limit:]
        
        # Add summary statistics to each entry
        for entry in recent_history:
            if entry.get("quality_assessment"):
                entry["quality_summary"] = entry["quality_assessment"].get("assessment", "No assessment")
            
            entry["decision_count"] = len(entry.get("decisions_made", []))
            entry["iteration_count"] = len(entry.get("agentic_history", []))
        
        return recent_history
    
    def clear_history(self):
        """Clear chat and execution history while preserving learned patterns"""
        self.chat_history.clear()
        self.execution_history.clear()
        # Keep learned patterns and error patterns for continued improvement
    ...
    def set_user_preference(self, key: str, value: Any):
        """Set user preferences for personalized behavior"""
...
class EnhancedAgentCLI:
    """Enhanced CLI interface with advanced features and insights"""
    
    def __init__(self, agent: EnhancedGeneralGroqAgent):
        self.agent = agent
        self.commands = {
            ...
            'decisions': self._show_decisions,
            'preferences': self._show_preferences,
            'set_pref': self._set_preference,
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                if user_input.lower().startswith('set_pref '):
                    self._handle_set_preference(user_input)
                    continue
                elif user_input.lower() in self.commands:
                    self.commands[user_input.lower()]()
                    continue
                
                # Process regular requests with timing
                start_time = datetime.now()
                ...
                # Try to get the latest generated content for display
                try:
                    if state.get('history'):
                        latest_results = state['history'][-1].get('results', [])
                        content_results = [r for r in latest_results if r.data and 'content' in r.data]
                        if content_results:
                            content = content_results[-1].data['content']
                            if len(content) > 200:  # Only show if substantial content
                                final_response += f"\n\n📄 Generated Content:\n{content}"
                except:
                    pass  # Fallback to just the final response
                
                print(f"\n🤖 Agent Response:\n{final_response}")
                
                # Show execution summary
                quality = state.get('quality_assessment', {}).get('overall_quality', 0)
                iterations = len(state.get('history', []))
                decisions = len(state.get('decisions_made', []))
                
                print(f"\n📊 Execution Summary: {execution_time:.1f}s | {iterations} iterations | {decisions} decisions | Quality: {quality:.1%}")
                print("\n" + "="*70)
                
            except KeyboardInterrupt:
                print("\n\n👋 Agent interrupted. Session summary:")
                self._show_session_summary()
                break
            except Exception as e:
                print(f"\n❌ An error occurred: {e}")
                print("Use 'status' to check agent state or 'help' for commands.")
    
    def _show_welcome(self):
        print("🚀 Enhanced AI Agent v2.0 - Advanced Decision Making & Multi-Step Planning")
        print("🌟 Enhanced Capabilities:")
        print("   💻 Intelligent code generation & development")
        print("   📁 Smart file & directory management")
        print("   🔍 Advanced research & web intelligence")
        print("   📊 Comprehensive data analysis & processing")
        print("   ✍️ Creative writing with quality assessment")
        print("   ⚙️ Complex workflow automation")
        print("   🧠 Multi-step decision making")
        print("   📈 Performance learning & adaptation")
        
        print("\n💡 Example Advanced Requests:")
        print("   • 'Create a complete Python web app with database and API'")
        print("   • 'Analyze my project files and suggest optimizations'")
        print("   • 'Research latest AI trends and create a comprehensive report'")
        print("   • 'Build an automated backup system with error handling'")
        print("   • 'Design a data processing pipeline with quality checks'")
        ...
        print("   preferences - Manage user preferences")
        print("   benchmark   - Run performance benchmark")
        
        print("\nType 'help' for full command list, 'quit' to exit")
        print("=" * 70)
    
    def _show_help(self):
        print("\n🔧 Enhanced Commands Available:")
        print("   help        - Show this comprehensive help message")
        print("   tools       - List all available tools with capabilities")
        print("   history     - Show detailed execution history")
        print("   clear       - Clear chat history (keeps learned patterns)")
        print("   status      - Show comprehensive agent status")
        print("   performance - Show performance metrics and statistics")
        print("   insights    - Show learning insights and patterns")
        print("   decisions   - Show recent decision history")
        print("   preferences - Show current user preferences")
        print("   set_pref    - Set user preference (e.g., 'set_pref quality_threshold 0.8')")
        print("   benchmark   - Run performance benchmark tests")
        print("   export      - Export session data and insights")
        print("   quit/exit   - Exit with session summary")
        print("\n💬 Or describe any complex task you want me to accomplish!")
        print("   The agent uses advanced decision making for optimal results.")
    
    def _show_tools(self):
        print("\n🛠️ Available Tools with Enhanced Capabilities:")
    ...
    def _show_history(self):
        ...
        for i, entry in enumerate(history, 1):
            task_type = "unknown"
            if entry.get('agentic_history'):
                task_type = entry['agentic_history'][0].get('analysis', {}).get('primary_task_type', 'unknown')
            
            quality = entry.get('quality_assessment', {}).get('overall_quality', 0)
            iterations = entry.get('iteration_count', 0)
            decisions = entry.get('decision_count', 0)
            exec_time = entry.get('execution_time', 0)
            
            status = "✅" if entry.get('goal_met') else "⚠️"
            
            print(f"\n   {i}. {status} {task_type.replace('_', ' ').title()}")
            print(f"      Request: {entry.get('request', '')[:80]}...")
            print(f"      Quality: {quality:.1%} | Time: {exec_time:.1f}s | Iterations: {iterations} | Decisions: {decisions}")
            
            if entry.get('quality_summary'):
                print(f"      Summary: {entry['quality_summary']}")
    ...
    def _show_status(self):
        ...
        print(f"   Decision History: {len(self.agent.decision_history)} decisions")
        print(f"   Learned Patterns: {sum(len(patterns) for patterns in self.agent.common_patterns.values())}")
        print(f"   Error Patterns: {len(self.agent.error_patterns)}")
        
        # Performance overview
        metrics = self.agent.performance_metrics
        success_rate = metrics["successful_tasks"] / max(metrics["total_tasks"], 1) * 100
        avg_quality = (sum(metrics["quality_scores"]) / max(len(metrics["quality_scores"]), 1)) if metrics["quality_scores"] else 0
        
        print(f"\n   Performance Overview:")
        print(f"   Success Rate: {success_rate:.1f}% ({metrics['successful_tasks']}/{metrics['total_tasks']})")
        print(f"   Average Quality: {avg_quality:.1%}")
        print(f"   Average Execution Time: {metrics['average_execution_time']:.1f}s")
    
    def _show_performance(self):
        insights = self.agent.get_performance_insights()
        print(f"\n📈 Performance Metrics & Analysis:")
        
        metrics = insights["performance_metrics"]
        print(f"   Total Tasks: {metrics['total_tasks']}")
        print(f"   Successful Tasks: {metrics['successful_tasks']}")
        print(f"   Success Rate: {(metrics['successful_tasks'] / max(metrics['total_tasks'], 1) * 100):.1f}%")
        print(f"   Average Execution Time: {metrics['average_execution_time']:.2f}s")
        print(f"   Average Quality Score: {insights['avg_quality_score']:.1%}")
        
        if metrics["quality_scores"]:
            recent_scores = metrics["quality_scores"][-5:]
            trend = "📈 Improving" if len(recent_scores) > 1 and recent_scores[-1] > recent_scores[0] else "📊 Stable"
            print(f"   Quality Trend: {trend}")
            print(f"   Recent Quality Scores: {[f'{score:.1%}' for score in recent_scores]}")
    
    def _show_insights(self):
        insights = self.agent.get_performance_insights()
        print(f"\n🧠 Learning Insights & Patterns:")
        
        patterns = insights["learned_patterns"]
        if patterns:
            print(f"   Learned Patterns by Task Type:")
            for task_type, count in patterns.items():
                print(f"   • {task_type.replace('_', ' ').title()}: {count} patterns")
        else:
            print("   No learned patterns yet - agent will learn from your interactions")
        
        errors = insights["error_patterns"]
        if errors:
            print(f"\n   Error Pattern Analysis:")
            print(f"   • Unique Error Types: {len(errors)}")
            most_common = max(errors.items(), key=lambda x: x[1]) if errors else None
            if most_common:
                print(f"   • Most Common Error Frequency: {most_common[1]} occurrences")
        
        print(f"\n   Decision Making:")
        print(f"   • Total Decisions Made: {insights['decision_history_count']}")
        
        # Show learning recommendations
        if insights['avg_quality_score'] < 0.7:
            print(f"\n   💡 Learning Recommendations:")
            print(f"   • Consider providing more detailed requirements")
            print(f"   • Break complex tasks into smaller steps")
            print(f"   • Use 'set_pref quality_threshold 0.8' for higher standards")
    
    def _show_decisions(self):
        decisions = self.agent.decision_history[-10:]  # Last 10 decisions
        
        if not decisions:
            print("\n🧠 No decisions recorded yet")
            return
        
        print(f"\n🧠 Recent Decision History ({len(decisions)} decisions):")
        for i, decision in enumerate(reversed(decisions), 1):
            print(f"\n   {i}. {decision.decision_type.value.replace('_', ' ').title()}")
            print(f"      Decision: {decision.decision}")
            print(f"      Confidence: {decision.confidence:.1%}")
            print(f"      Reasoning: {decision.reasoning[:100]}...")
            if decision.alternatives:
                print(f"      Alternatives: {', '.join(decision.alternatives[:2])}")
    
    def _show_preferences(self):
        prefs = self.agent.get_user_preferences()
        
        print(f"\n⚙️  Current User Preferences:")
        if prefs:
        ...
        print(f"   • quality_threshold: Minimum quality score (0.0-1.0)")
        print(f"   • max_iterations: Maximum planning iterations")
        print(f"   • preferred_tools: Comma-separated tool preferences")
        print(f"   • output_format: 'detailed' or 'concise'")
        print(f"   • auto_retry: 'true' or 'false' for automatic retries")
        
        print(f"\n   Usage: set_pref <key> <value>")
        print(f"   Example: set_pref quality_threshold 0.8")
    
    def _handle_set_preference(self, command: str):
        parts = command.split(' ', 2)
        if len(parts) != 3:
            print("❌ Usage: set_pref <key> <value>")
            return
        
        _, key, value = parts
        
        # Type conversion for known preferences
        if key in ['quality_threshold']:
            try:
                value = float(value)
                if not 0.0 <= value <= 1.0:
                    print("❌ quality_threshold must be between 0.0 and 1.0")
                    return
            except ValueError:
                print("❌ quality_threshold must be a number")
                return
            ...
            try:
                ...
                print("❌ max_iterations must be an integer")
                return
        elif key == 'auto_retry':
            if value.lower() not in ['true', 'false']:
                print("❌ auto_retry must be 'true' or 'false'")
                return
            value = value.lower() == 'true'
        
        self.agent.set_user_preference(key, value)
        print(f"✅ Preference set: {key} = {value}")
    
    def _set_preference(self):
        print("Use: set_pref <key> <value>")
        print("See 'preferences' command for available options")
    ...
    def _run_benchmark(self):
        ...
        benchmark_tasks = [
            ...
            "Generate a short creative story about robots",
            "Calculate statistics for numbers: 1 2 3 4 5"
        ]
        
        results = []
        for i, task in enumerate(benchmark_tasks, 1):
            print(f"   Running benchmark {i}/{len(benchmark_tasks)}: {task[:50]}...")
            
            start_time = datetime.now()
            state = self.agent.process_request(task)
            end_time = datetime.now()
            
            execution_time = (end_time - start_time).total_seconds()
            quality = state.get('quality_assessment', {}).get('overall_quality', 0)
            success = state.get('goal_met', False)
        ...
        print(f"   Average Time per Task: {total_time/len(results):.1f}s")
        print(f"   Average Quality: {avg_quality:.1%}")
        print(f"   Success Rate: {success_rate:.1%}")
        
        for i, result in enumerate(results, 1):
            status = "✅" if result['success'] else "❌"
            print(f"   {i}. {status} {result['execution_time']:.1f}s | Quality: {result['quality']:.1%}")
    ...
    def _export_data(self):
        ...
        export_data = {
            ...
            'execution_history': self.agent.get_execution_history(10),
            'learned_patterns': self.agent.common_patterns,
            'user_preferences': self.agent.get_user_preferences(),
            'insights': self.agent.get_performance_insights()
        }
        
        filename = f"agent_session_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def _clear_history(self):
        ...
        print("\n🗑️ Chat history cleared (learned patterns preserved)")
    
    def _exit(self):
        print("\n👋 Thanks for using the Enhanced AI Agent!")
        self._show_session_summary()
        exit(0)

# Create alias for backward compatibility  
AgentCLI = EnhancedAgentCLI

# Enhanced main execution
def main():
    """Enhanced main function with better error handling and setup"""
    ...
    try:
        ...
        agent = EnhancedGeneralGroqAgent(groq_api_key)
        
        print("   🖥️  Initializing enhanced CLI...")
        cli = EnhancedAgentCLI(agent)
        ...
        print(f"\n❌ Failed to initialize enhanced agent: {e}")
