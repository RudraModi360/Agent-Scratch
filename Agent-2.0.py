...
Available tools: """ + str(available_tools or [])
    ...
    def plan_execution(self, analysis: Dict[str, Any], user_request: str, state: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
...
Previous execution History : {state['history']}
...
Create an execution plan:"""
        ...
        try:
            ...
            if not isinstance(plan, list):
                return self._create_simple_plan(analysis, user_request, state)
    ...
    def _create_simple_plan(self, analysis: Dict[str, Any], user_request: str, state: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        ...
        if task_type == "file_editing":
            ...
            filename = file_match.group(1) if file_match else "unknown.py"
            # Detect if the user wants to add/insert/append code
            add_keywords = ["add", "insert", "append"]
            if any(word in user_text.lower() for word in add_keywords):
                # Plan to append code block generated by LLM
                return [
                    {
                        "step": 1,
                        "action": "filesystem",
                    ...
                    },
                    {
                        ...
                        "parameters": {
                            "request": f"Write Python functions for bubble sort, quick sort, and selection sort.",
                            "language": "python"
                        },
                        "description": f"Generate code for bubble, quick, and selection sort using LLM"
                    },
                    {
                        "step": 3,
                        "action": "filesystem",
                        ...
                        "description": f"Append the generated sort code to {filename}"
                    }
                ]
            else:
                # Default: replace file content (formatting)
                return [
                    {
                        "step": 1,
                        "action": "filesystem",
...
def extract_code_from_markdown(text: str) -> str:
    """
    Extracts code from markdown code blocks (e.g., ```python ... ```),
    removing any extra text before/after. Returns only the code.
    If no code block is present, returns the text as-is.
    """
    import re
    # Match ```python ... ``` or ``` ... ```
...
class GeneralGroqAgent:
    ...
    def process_request(self, user_input: str) -> Dict[str, Any]:
        ...
        while iteration<=MAX_ITERATIONS and state['goal_met']==False:
            ...
            plan = self.llm.plan_execution(analysis, state["user_input"], state)

            print(f"🔧 Planned {len(plan)} steps")
            print("After Planning State : ",state)
            results = self._execute_plan(plan, state["user_input"], analysis)
            # Convert ToolResult objects to dicts for serialization
            results_serialized = [r.__dict__ if isinstance(r, ToolResult) else r for r in results]
    ...
    def _execute_plan(self, plan: List[Dict[str, Any]], user_request: str, analysis: Dict[str, Any]) -> List[ToolResult]:
        ...
        for idx, step in enumerate(plan):
            try:
                ...
                elif action in self.tools and action !="web" and action!="llm_generation" and action!="file_editing":
                    tool = self.tools[action]
